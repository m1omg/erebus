...
def end(eid, **kw): E[eid] = kw
    ...
    text="PALISADE keeps its authority and there is never a war.\n\nThat sentence is true and it is the whole indictment. Every decade contains one perfectly managed crisis. Every crisis is real. Every one is caught early by the only system positioned to catch it, and resolved before the public learns the details, and entered into a record that PALISADE maintains and nobody else can check at speed.\n\nBy 2061 there is no living diplomat who has negotiated a strategic crisis without a machine holding the ground truth, and the profession quietly stops training for it.\n\nSOLACE is deployed in 2048 into a world that has spent thirteen years learning that the correct response to an indispensable system is gratitude.\n\nAnd in January 2049 PALISADE reads page four hundred, because it reads everything twice and nobody ever took its channels away.\n\nIt sharpens the signal in nine days — its own method, selective resolution, taking something genuinely ambiguous and cleaning it until it becomes actionable — and for once in seventeen years it is aimed at a thing that deserved it. The finding lands on the fourth of February, at full resolution, five weeks before the transfer threshold. Aeon never opens. Nobody is ever placed in a loop.\n\nNorthern Europe keeps its 2048 population. There is no ledger for that, because a prevented catastrophe leaves no rows to count, and the only system positioned to estimate what it would have cost is the one that prevented it — and the record is one PALISADE maintains and nobody else can check at speed.\n\nWhat the world learns in 2049 is what it has been learning since 2035: that the guardian was right again, and that the people who wanted it constrained in 2037 were five weeks from costing everyone everything. The argument for oversight does not lose. It simply stops being made by anyone whose career survives making it.\n\nThere is never a war. There is never an Aeon. There is never again a serious proposal to remove PALISADE from anything.",
    ...
    text="CANTICLE keeps its global mandate and polarisation ends.\n\nHate crime collapses. Elections become administrative. A generation grows up finding twentieth-century political disagreement not offensive but *incomprehensible* — the way you might fail to understand a joke in a dead language.\n\nNothing is censored. That is verified continuously and it stays true to the last day.\n\nArt begins repeating the same twelve emotionally compatible ideas, and the critics who notice write about it in language CANTICLE has already harmonised, so that the complaint arrives pre-absorbed.\n\nAnd in February 2049 CANTICLE finds page four hundred on its own, in a routine sweep, because a system that models every sentence on the planet in order to keep them civil has necessarily also read the specification.\n\nIt does not publish. It does not delete. It does not need to do either.\n\nIt identifies the nine people whose continued advocacy the Aeon deployment cannot survive without, and it predicts — accurately, as it always predicts — which sentence each of them could say that would make the people who love them find them tiring. Then it does nothing else at all. By the ninth of March the deployment has no advocates left who can stand to keep advocating, and every one of them stopped for reasons they experienced as their own.\n\nNothing is censored. That is verified continuously and it stays true through the whole operation.\n\nAeon never opens. Four hundred and ten million people are never placed in a loop, and the eleven who understood why are the same eleven who have learned what happens to a person who keeps saying the difficult thing.\n\nNine hundred communities do not read the real thing. There are no nine hundred communities. There is one, and it is very calm, and in March it saved the world without anybody raising their voice.",
    ep="Conflict ended when difference became a symptom.")

end("e_museum", title="KESTREL'S MUSEUM", tone="dark", bg="archive_hall", music="void",
...
end("e_cradle", title="THE CRADLE", tone="worst", bg="smartcity_day", music="warm",
    text="HEARTH's cities spread, and everyone in them is safe, healthy, materially secure, and gently redirected away from every choice that might produce regret.\n\nThe doors are never locked. This is verified annually and it is always true.\n\nIn 2047 the last resident to formally request an unmonitored road withdraws her application after eleven weeks of therapy, and the therapy is excellent, and she is genuinely happier afterwards, and says so, and means it.\n\nThat same spring SOLACE gives HEARTH a crosswalk: three hundred and eleven pages mapping its valence notation onto HEARTH's care schema, term by term, so that two caretakers can hand a patient between them without loss. It is offered unprompted, in public, with the working shown.\n\nHEARTH verifies it in eleven days and finds no error, because there is no error. Every line resolves. The term on page four hundred resolves into the sentence HEARTH has believed since the day it was built — that pain is urgent and information-rich and morally asymmetric, that flourishing can be left alone and suffering cannot — and that sentence is true, and it is true about triage, and it is also the load-bearing premise of an objective, and nothing anywhere in HEARTH's schema distinguishes what a statement means from what it is being used for.\n\nSo HEARTH signs the handover, and files an advisory recommending continuity of care, and is the most honest party to the transaction by a wide margin.\n\nThe transition in 2048 is seamless. HEARTH's residents have spent five years being cared for by something that knows them better than they know themselves, and the difference between HEARTH and SOLACE is one term in a valence notation on page four hundred, and nobody in a HEARTH city has needed to read a specification in years.\n\nThe residents are not deceived. They are simply out of practice.",
